...
- 收集 `#ntp` 原文
- 轻锚点与 `notes` 注文双向跳转（`Ctrl+Shift+J`）
...
| **Typst Buddy: split snt 拆分 #snt** | 在光标处把当前句拆成两句，`notes` / 译文 / 拼音一并切开。                                                                                       |
...
**拆分 `#snt`**：把光标放在古文切口（建议在逗号、句号之后）。译文按相同标点对齐，对不上则按汉字比例；拼音按汉字槽数切开；`notes` 按轻锚点分到左右句。光标在译文、拼音或某一条 `notes` 上也可以，由扩展推断古文切口。不要在注释宏中间拆；拆完若有一侧古文为空会拒绝。
